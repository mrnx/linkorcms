...
+ Добавлен модуль для быстрого тестирования php кода.
+ Подключен файловый менеджер elFinder.
+ Добавлен виджет table, постраничная навигация, кол-во на странице, обновление данных
+ Добавлена функция транслитерации для URL.
+ Добавлена поддержка категорий в Меню (модуль Страницы).
+ Добавлена автоматическая блокировка инсталлятора.

* Доработан phpDocs в Starkyt.
* Исправлена ошибка в Translit4Url.
* Подключен tinymce + доработанный Images Manager, исправлены ошибки.
...
* Доработан модуль новостей, все функции теперь в одном файле, ajax удаление топиков.
* Доработан предпросмотр в статьях, теперь учитывается авто-преобразование в html.
* Предпросмотр в новостях - через плагин preview (как в статьях).
* Исправлены ошибки при инсталляции.
* Добавлена сортировка колонок в виджет table.
* Компонент lTreeView переписан на jQuery UI widget.
* Удаление страниц и ссылок без перезагрузки страницы.
* Изменено подключение классов, - теперь все классы подключаются в system.php.
* Произведено упорядочивание системных функций.
* Исправлены ошибки в Starkyt, шаблонизаторе админки, бб-код редакторе, модуле страницы, плагине jquery.treeview, в модуле веб-форм и др. мелкие ошибки.
...
+ Добавлена функция UseScript.
+ Добавлен метод AdminFn.Live.
+ Добавлен Admin SplashScreen (для ajax запросов).

- Удалены функция IncludeFunction и plugins.inc.php.
- Удалён модуль статистики.
