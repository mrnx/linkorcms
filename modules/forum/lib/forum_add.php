...
function Forum_Add_AddPost2( $topic = null ) {
	global $db, $config, $user, $forum_lang;
	$er = array();
	$text = '';
	if($topic == null) {
		if(isset($_GET['topic'])) {
			...
			$er[] = $forum_lang['error'].': !isset($_GET[\'topic\']).';
		}
	}else {
		$object = $topic;
	}
	if((!isset($_POST['name'])
		...
		if($user->Auth) {
			...
			$er[] =$forum_lang['no_reg_add'];
		}
		$text = SafeEnv($_POST['text'], 0, str);
		if(strlen($text) == 0) {
			$er[] =$forum_lang['error_no_message'];
		}
	}
	if(!$user->Get('u_add_forum')) {
		$er[] =$forum_lang['blocking'];
	}
	if(count($er) == 0) {
		...
		if( count($mdb)>0 and $mdb[0]['delete'] == 0 or  $config['forum']['basket'] == false) {
			if($mdb[0]['close_topics'] == 1) {
				$er[] = $forum_lang['topic_close_for_discussion'].'.'.$forum_lang['no_create_new_message_current_topic_add'];
				return $er;
			}
			$m_time	= time();
			...
			$er[] = $forum_lang['topic_basket'].'.'.$forum_lang['no_topic_basket_edit'];
			return $er;
		}
	}
}
