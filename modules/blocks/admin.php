...
function AdminBlocksEdit( $a ){
	global $config, $site, $db, $user; // для совместимости со тарыми версиями блоков НЕ УДАЛЯТЬ
	$text = '';
	$title = '';
	$showin = array();
    $extrauri = '';
...
function AdminBlocksSave( $a ){
	...
	if($a == 'newsave'){
		...
		if($block_pos != $_POST['position'][0]){ // Пересчитываем все place в предыдущем position
